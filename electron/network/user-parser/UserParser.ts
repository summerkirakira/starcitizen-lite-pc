...
export async function getUser(login_id: number, email: string, password: string): Promise<User> {
    ...
    const spent_lines = billing_page.querySelectorAll('.spent-line')
    console.log(spent_lines)
    const lastElement = Array.from(spent_lines).pop();
    const total_spent = parseFloat(lastElement.querySelector('em').textContent.replace('$', '').replace('USD', '').replace(",", '').trim()) * 100
