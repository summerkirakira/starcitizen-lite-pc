...
const ENDPOINT = 'api2'
const PARAMS = 'ar=1&k=6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o&co=aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM.&hl=zh-CN&v=vP4jQKq0YJFzU6e21-BGy3GP&size=invisible&cb=lam2123evvtl'

const K = '6LerBOgUAAAAAKPg6vsAFPTN66Woz-jBClxdQU-o'
const CO = 'aHR0cHM6Ly9yb2JlcnRzc3BhY2VpbmR1c3RyaWVzLmNvbTo0NDM.'
const V = 'vP4jQKq0YJFzU6e21-BGy3GP'


const BASE_URL = 'https://recaptcha.net/recaptcha/'
const CONTENT_TYPE = 'application/x-www-form-urlencoded'

const firstStagePattern = new RegExp('"recaptcha-token" value="(.*?)"');

const finalStagePattern = new RegExp('"rresp","(.*?)"');
...
function getPostData(token: string): string {
...
export async function getFirstStageToken(): Promise<string> {
    const { data } = await axioInstance.get(`/${ENDPOINT}/anchor?${PARAMS}`)
    const match = data.match(firstStagePattern)
    return match[1]
}

export async function getSecondStageToken(token) {
    const postData = getPostData(token)
    const { data } = await axioInstance.post(`/${ENDPOINT}/reload?k=${K}`, postData)
    return data
}

export async function getRecaptchaToken(): Promise<string> {
    const firstStageToken = await getFirstStageToken()
    const secondStageToken = await getSecondStageToken(firstStageToken)
    return secondStageToken.match(finalStagePattern)[1]
}